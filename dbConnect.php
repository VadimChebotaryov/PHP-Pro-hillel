...
class dbConnect
{
    ...
    public function __construct($dbname = 'blog',
                                $username = 'Vadim',
                                $password = 'QWERTY',
                                $host = 'localhost')
    {
    ...
    public function tableExists(string $table): bool {
        try {
            $this->db->query("SELECT * FROM $table LIMIT ");
        } catch (PDOException) {
            return false;
        }
        return true;
    ...
    {
        $query = $this->db->prepare("CREATE TABLE $tableName 
        (id int unsigned auto_increment primary key,
        name varchar(50) not null,
        surname varchar(70) not null, 
        age int not null, email varchar(256) not null unique);");
        $query->execute();
    }

    public function createUser(string $name, string $surname, int $age, string $email): void
    {
        $query = $this->db->prepare("INSERT INTO users(name, surname, age, email)
